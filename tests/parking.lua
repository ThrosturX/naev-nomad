...
local ok, reason = parking.validate(true, false, 89.999, 100)
assert(not ok and reason:find("90%%"),
   "parking must reject shields below the configured threshold")
assert(parking.validate(true, false, 90, 100),
   "parking must accept shields exactly at the configured threshold")
assert(parking.validate(true, false, 0, 0),
   "parking must accept a carrier with no fitted shield capacity")
assert(not parking.validate(true, false, 0, 100),
   "parking must reject depleted shields when capacity remains")
assert(not parking.validate(false, false, 100, 100),
...
assert(record.system == 'A&B "System"' and record.x == 123.5
   ...
   and record.diff == 'Nomad Park 1&2' and record.carrier == 'Home',
